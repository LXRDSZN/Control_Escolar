...
import {
  ...
  obtenergrupos,
  obtenermaterias,
  alta_horario, // Importa la función para registrar horarios
...
const registrarNuevoHorario = async () => {
  ...
  try {
    ...
    // Llamar a la API para registrar el horario
    const response = await alta_horario(
    ...
    setTimeout(() => {
        window.location.href = `/Horarios`; // Redirigir a la página del panel
      }, 750);
  } catch (error) {
    console.error('Error al registrar el horario:', error);
    toast.error('Hubo un problema al registrar el horario. Intenta nuevamente.', {
      position: 'top-right',
      duration: 5000,
    });
  }
  ...
  <div class="syste-panel">
    <div class="panel">
      ...
      <div class="boton-agregar-container">
        <button @click="registrarNuevoHorario" class="btn btn-add">
          Registrar Horario
        </button>
      </div>
    </div>
